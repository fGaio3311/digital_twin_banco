...
# Create PDF
pdf_file = "sqlite_test_report.pdf"
...
# Add title
title = Paragraph("SQLite Load Test Results Report", title_style)
elements.append(title)
elements.append(Spacer(1, 20))

# Add test description
description_text = """This report presents the performance metrics of our digital twin application using SQLite as the database backend.
The test was conducted using Locust, simulating 10 concurrent users with a spawn rate of 1 user per second over a 1-minute period.
The test focused on basic API operations with minimal load to evaluate baseline system responsiveness.
Note: Even with minimal load, SQLite showed significant connection handling issues, which is expected for a file-based database under concurrent access."""
...
# Load test data
with open('sqlite_test_results.json', 'r') as f:
    data = json.load(f)
...
endpoint_data = [
    ["Endpoint", "Total Requests", "Success Rate (%)", "Avg Response (ms)", "Failures"],
]
...
for endpoint, details in data["endpoint_details"].items():
    endpoint_data.append([
        ...
        format_number(details["failures"]),
    ])
